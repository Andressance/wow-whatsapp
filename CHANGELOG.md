...
- WhatsApp is told which game and client you are on, your character (name, realm, level, race, class, faction, guild), zone and map coordinates, money, talents and professions. The addon sends these few lines with its hello and again when they change (a `c` flag and an extra field in the strip record), the bridge keeps the latest in `state.json` and passes it to every run with `--append-system-prompt`. `/wow-whatsapp context` shows it, `/wow-whatsapp context off` stops it (and clears the bridge's copy), `"gameContext": false` in `config.json` disables it on the bridge side.
...
- Deleting a chat in game now tells the bridge to forget its transcript and WhatsApp session (a `d` strip record), so a later restore no longer brings the chat back. Deletions made while the bridge was away are resent with the next hello.
...
- In-game chat window (`/wow-whatsapp`) with multiple chats, each backed by its own persistent WhatsApp session, running in parallel up to `maxParallel`.
...
- Live progress in the working bubble: action count, elapsed time, and the files and commands WhatsApp is touching.
- Replies echoed into the game chat; `/r` replies to WhatsApp when it was the last to message you; `/ai <text>` sends from the chat box.
- **Allow & retry** button when WhatsApp is denied a tool, which appends the rule to `allowedTools` and resumes.
- Per-chat working folder (`/wow-whatsapp cd`, **Folder** button) resolved against the bridge's default folder.
- Bridge-side transcripts and automatic restore of chats after the client wipes addon saved data.
- `wow-whatsapp` command (`npm link`) that uses the folder it is started from as the default project.
...
[Unreleased]: https://github.com/chelinho139/wow-whatsapp/compare/v0.3.0...HEAD
[0.3.0]: https://github.com/chelinho139/wow-whatsapp/releases/tag/v0.3.0
